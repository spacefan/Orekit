...
public class EcksteinHechlerPropagatorTest {
    ...
    @Test
    public void testShiftedBy() throws OrekitException {
        final KeplerianOrbit orbit =
            new KeplerianOrbit(7.8e6, 0.032, 0.4, 0.1, 0.2, 0.3, PositionAngle.TRUE,
                               FramesFactory.getEME2000(), AbsoluteDate.J2000_EPOCH, 3.986004415e14);
        EcksteinHechlerPropagator propagator =
            new EcksteinHechlerPropagator(orbit, provider);
        double dt = 10.0;
        EcksteinHechlerPropagator.EHOrbit full = propagator.propagateOrbit(orbit.getDate().shiftedBy(1000));
        EcksteinHechlerPropagator.EHOrbit fullShifted = full.shiftedBy(dt);

        // filter out the non-Keplerian terms that remain in the full orbit generated by Eckstein-Propagator
        CircularOrbit filtered   = new CircularOrbit(full.getA(),
                                                     full.getCircularEx(),
                                                     full.getCircularEy(),
                                                     full.getI(),
                                                     full.getRightAscensionOfAscendingNode(),
                                                     full.getAlphaM(), PositionAngle.MEAN,
                                                     full.getFrame(), full.getDate(), full.getMu());
        CircularOrbit filteredShifted = filtered.shiftedBy(dt);

        EcksteinHechlerPropagator.EHOrbit propagated  = propagator.propagateOrbit(fullShifted.getDate());

        // shifting the full orbit (with non-Keplerian terms) is quite good
        Assert.assertEquals(propagated.getA(),                             fullShifted.getA(),                             0.0005);
        Assert.assertEquals(propagated.getCircularEx(),                    fullShifted.getCircularEx(),                    2.0e-10);
        Assert.assertEquals(propagated.getCircularEy(),                    fullShifted.getCircularEy(),                    2.0e-10);
        Assert.assertEquals(propagated.getI(),                             fullShifted.getI(),                             1.0e-10);
        Assert.assertEquals(propagated.getRightAscensionOfAscendingNode(), fullShifted.getRightAscensionOfAscendingNode(), 1.0e-9);
        Assert.assertEquals(propagated.getAlphaM(),                        fullShifted.getAlphaM(),                        1.0e-9);

        // shifting a filtered orbit is not as good
        Assert.assertTrue(FastMath.abs(propagated.getA()                             - filteredShifted.getA())                             > 12);
        Assert.assertTrue(FastMath.abs(propagated.getCircularEx()                    - filteredShifted.getCircularEx())                    > 5e-6);
        Assert.assertTrue(FastMath.abs(propagated.getCircularEy()                    - filteredShifted.getCircularEy())                    > 4e-7);
        Assert.assertTrue(FastMath.abs(propagated.getI()                             - filteredShifted.getI())                             > 1e-6);
        Assert.assertTrue(FastMath.abs(propagated.getRightAscensionOfAscendingNode() - filteredShifted.getRightAscensionOfAscendingNode()) > 1e-5);
        Assert.assertTrue(FastMath.abs(propagated.getAlphaM()                        - filteredShifted.getAlphaM())                        > 1e-5);

    }

    @Test
    public void testInterpolateEHOrbit() throws OrekitException {
        checkInterpolate(new OrbitFilter() {
            public CircularOrbit filter(EcksteinHechlerPropagator.EHOrbit orbit) {
                return orbit;
            }
        }, 4.7e-8, 3.8e-15, 6.9e-14, 6.2e-15, 3.1e-14, 2.0e-14);
    }

    @Test
    public void testInterpolateNonEHOrbit() throws OrekitException {
        // if we remove non-Keplerian derivatives before interpolation,
        // we cannot interpolate anymore
        try {
            checkInterpolate(new OrbitFilter() {
                public CircularOrbit filter(EcksteinHechlerPropagator.EHOrbit orbit) {
                    // create a regular CircularOrbit, without derivatives
                    return new CircularOrbit(orbit.getA(),
                                             orbit.getCircularEx(),
                                             orbit.getCircularEy(),
                                             orbit.getI(),
                                             orbit.getRightAscensionOfAscendingNode(),
                                             orbit.getAlphaM(), PositionAngle.MEAN,
            ...
            Assert.assertEquals(OrekitMessages.ORBIT_TYPE_MISMATCH, oe.getSpecifier());
            Assert.assertEquals(CircularOrbit.class.getName(), oe.getParts()[0]);
            Assert.assertEquals(EcksteinHechlerPropagator.EHOrbit.class.getName(), oe.getParts()[1]);
        }
    }

    public void checkInterpolate(OrbitFilter f, double expectedDa, double expectedDex, double expectedDey,
                                 double expectedDi, double expectedDraan, double expectedDalpha)
        ...
        sample.add(f.filter(propagator.propagateOrbit(orbit.getDate().shiftedBy( 60.0))));
        sample.add(f.filter(propagator.propagateOrbit(orbit.getDate().shiftedBy(120.0))));

        CircularOrbit propagated   = (CircularOrbit) (propagator.propagate(orbit.getDate().shiftedBy(90.0)).getOrbit());
        CircularOrbit interpolated = propagated.interpolate(propagated.getDate(), sample);

        // shifting the full orbit (with non-Keplerian terms) is quite good
        Assert.assertEquals(expectedDa,     FastMath.abs(propagated.getA()                             - interpolated.getA()),                             0.1 * expectedDa);
        Assert.assertEquals(expectedDex,    FastMath.abs(propagated.getCircularEx()                    - interpolated.getCircularEx()),                    0.1 * expectedDex);
        Assert.assertEquals(expectedDey,    FastMath.abs(propagated.getCircularEy()                    - interpolated.getCircularEy()),                    0.1 * expectedDey);
        Assert.assertEquals(expectedDi,     FastMath.abs(propagated.getI()                             - interpolated.getI()),                             0.1 * expectedDi);
        Assert.assertEquals(expectedDraan,  FastMath.abs(propagated.getRightAscensionOfAscendingNode() - interpolated.getRightAscensionOfAscendingNode()), 0.1 * expectedDraan);
        Assert.assertEquals(expectedDalpha, FastMath.abs(propagated.getAlphaM()                        - interpolated.getAlphaM()),                        0.1 * expectedDalpha);

    }

    private interface OrbitFilter {
        public CircularOrbit filter(final EcksteinHechlerPropagator.EHOrbit orbit) throws OrekitException;
    }
