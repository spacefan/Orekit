    ...
    public void testIncreasingError() throws OrekitException {
        ...
        for (EventEntry entry : log) {
            ...
            if (entry.name.contains("10s")) {
                Assert.assertTrue(error > 0.000003);
                Assert.assertTrue(error < 0.000007);
            } else if (entry.name.contains("100s")) {
                Assert.assertTrue(error > 0.005);
                Assert.assertTrue(error < 0.008);
            } else if (entry.name.contains("1000s")) {
                Assert.assertTrue(error > 1.5);
                Assert.assertTrue(error < 4.7);
            }
        }
    }
